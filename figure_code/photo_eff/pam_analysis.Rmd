...
```
# Figure with different wounds on different panels, colored by fish. 
...
ggsave(
  here("figures", "pam", "photoeff_vs_wound_violin_no_small_large.png"),
  width = 7, height = 5, dpi = 150
)
...
```{r}
pam_line_plot <- ggplot(pam_undisturbed_healed_tissue_wound_no_wound,  aes(x = fish, y = fv_fm, color = fish)) +
  ...
  stat_summary(fun.data = median_hilow, geom = "errorbar", width = 0, size = 1) +
  scale_color_manual(values = fish_palette) +
...
lrt_interaction_algae <- anova(mod_add, mod_int_algae, test = "Chisq")
