...
## Creating lmer models
```{r}
mod_int_p <- lmer(prop ~ fish * wound + (1 | tank), data = percent_closure )
# anova(fish_glm, type="II", test.statistic="F")


mod_add_p <- lmer(prop ~ fish + wound + (1|tank), data = percent_closure)
# anova(wound_glm, test = "Chisq")

mod_fish_p <- lmer(prop ~ fish + (1| tank), data = percent_closure )
# anova(fish_glmer, type="II", test.statistic="F")

mod_wound_p <- lmer(prop ~ wound + (1|tank), data = percent_closure)

mod_null_p <- lmer(prop ~ 1 + (1|tank), data = percent_closure)
...
lrt_table_p %>%
  gt() %>%
  tab_header(
    title = "Table #. Likelihood Ratio Tests for Effects on Final Proportion Healed",
    #subtitle = paste("Scaled growth:", expression((final - initial)/initial^b))
  ) %>%
  cols_label(
    Test = "Model Comparison",
...
### quick analysis
```{r}
mod <- lmer(percent.healed ~ fish * wound + (1|tank), data = percent_closure)

summary(mod)
anova(mod)

# Note, see significant fish effect = 0.0362 YESSS!
```
