...
### ashely algae healing figure
```{r}
# highlight_ids <- c("56", "58", "69", "91", "95", "103", "111", "121")
# percent_closure %>% 
#   ggplot(aes(wound, healing_rate, color = fish))+
#   geom_point()+
#   geom_point(data = subset(percent_closure, coral_id %in% highlight_ids), 
#              color = "blue", 
#              size = 3)+
#   geom_jitter(
#     position = position_jitterdodge(
...
percent_closure$coral_id <- as.character(percent_closure$coral_id)

percent_closure$highlight <- ifelse(
  percent_closure$coral_id %in% highlight_ids,
  "Highlight",
  "Other"
)

table(percent_closure$highlight)


library(ggplot2)

ggplot(percent_closure,
       aes(x = wound,
           y = healing_rate,
           color = fish)) +
  geom_jitter(
    aes(size = highlight, alpha = highlight),
    position = position_jitterdodge(
      jitter.width = 0.1,
      dodge.width  = 0.6
    )
  ) +
  scale_size_manual(values = c("Other" = 2.5,
                               "Highlight" = 4)) +
  scale_alpha_manual(values = c("Other" = 0.4,
                                "Highlight" = 1)) +
  theme_classic()
```

### Algae effect on healing rate?
```{r}
highlight_ids <- c("56","58","69","91","95","103","111","121")

percent_closure$coral_id <- as.character(percent_closure$coral_id)

percent_closure$algae <- ifelse(
  percent_closure$coral_id %in% highlight_ids,
  "Yes",
  "No"
)
percent_closure$algae <- factor(percent_closure$algae,
                                levels = c("No", "Yes"))
mod_base <- lmer(healing_rate ~ fish * wound + (1 | tank),
                 data = percent_closure,
                 REML = FALSE)

mod_add_algae <- lmer(healing_rate ~ fish * wound + algae + (1 | tank),
                      data = percent_closure,
                      REML = FALSE)

anova(mod_base, mod_add_algae)

mod_int_algae <- lmer(healing_rate ~ fish * wound * algae + (1 | tank),
                      data = percent_closure,
                      REML = FALSE)

anova(mod_add_algae, mod_int_algae)


#within large wound

subset_nf <- subset(percent_closure, fish == "No Fish")

lmer(healing_rate ~ algae + wound + (1 | tank),
     data = subset_nf)
